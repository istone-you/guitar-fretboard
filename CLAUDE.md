...
- **出題形式**: クイズモードでは以下の種類を切り替えられること。
  ...
  - スケール・指板
- **コード・識別**:
  - 指板上にコードフォームを表示し、`A〜G` のルート選択とコード種別選択の2段階で回答できること。
  - コード種別の出題範囲は複数選択で絞り込めること。
- **スケールクイズ**:
  - スケールクイズではクイズパネル内からスケール種類を切り替えられること。
  - スケール・音名選択は構成音を複数選択すること。
  - スケール・指板は表示範囲内の該当セルをすべて選ぶこと。
- **進行**:
  - 回答後は自動遷移せず、明示的なボタンで次へ進むこと。
  - スケールクイズのみ `次へ` ではなく `もう一度` と表示し、同じ問題をリトライすること。
...
- **設定永続化**: 設定は localStorage に保持すること。
  - 対象には少なくともテーマ、臨時記号、言語設定、表示サイズを含めること。
